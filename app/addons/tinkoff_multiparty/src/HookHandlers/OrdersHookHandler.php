...
class OrdersHookHandler
{
     ...
     *
     * Action performed:
     *    - Allows to manage order status at returning from Tinkoff Multiparty.
     *
     ...
     *
     * Actions performed:
     *     - Creates a data set for the closing receipt for Tinkoff Multiparty
    ...
    public function onChangeOrderStatusPost(
        ...
        if (
            ...
            || !empty($order_info['payment_info']['addons.tinkoff_multiparty.closing_receipt_was_sended'])
        ) {
            return;
        }
        ...
        if (empty($response['Success'])) {
            ...
            fn_update_order_payment_info(
                ...
                [
                    'addons.tinkoff_multiparty.closing_receipt_was_sended' => __('no')
                ]
            );
        } else {
            fn_set_notification(NotificationSeverity::NOTICE, __('notice'), __('addons.tinkoff_multiparty.closing_receipt_was_sended'));
            fn_update_order_payment_info(
                (int) $order_info['order_id'],
                [
                    'addons.tinkoff_multiparty.closing_receipt_was_sended' => __('yes')
                ]
            );
        }
    }
