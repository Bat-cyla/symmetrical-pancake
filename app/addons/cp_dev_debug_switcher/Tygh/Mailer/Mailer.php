...
class Mailer
{
    ...
    public function send($message, $area = null, $lang_code = null, array $transport_settings = null)
    {
        ...
        if (Registry::get('addons.cp_dev_debug_switcher.save_mail_message') == 'Y') {
            ...
            if (!$message instanceof Message) {
                ...
                $_message = $builder->createMessage($message, $area, $lang_code);
            }
            
            $message_content = [
                'body' => $_message->getBody(),
                'from' => $_message->getFrom(),
                'to' => $_message->getTo()
            ];
            
            return fn_cp_dev_debuger_mailer_changes($message, $area, $lang_code);
        } else {
