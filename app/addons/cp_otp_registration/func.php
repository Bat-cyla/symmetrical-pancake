...
function fn_cp_otp_registration_dispatch_before_display()
{
    if (fn_notification_exists('extra', 'default_checkout_exists_popup')) {
        ...
        if (!empty($ajax_vars['cp_guest_order'])) {
            fn_delete_notification('default_checkout_exists_popup');
        }        
    }
}
...
function fn_cp_otp_registration_update_user_pre($user_id, &$user_data, $auth, $ship_to_another, &$notify_user)
{
    ...
    if (empty($user_id) && fn_cp_otp_check_verified($user_data['phone'],'register')) {
...
function fn_cp_otp_send_code($user_data, $type = 'register', &$error = '')
{
    ...
    if (!empty($user_data[$cpv1])) {
        ...
        if (function_exists('fn_cp_sms_send_by_service')
            ...
            if (!defined('CP_OTP_DEMO_MODE')) {
                $settings = Registry::get('addons.cp_otp_registration');
                $sms_service = Registry::get('addons.cp_sms_services.service');             
                if (
                    $settings['login_type'] == 'otp' && 
                    !empty($settings['call_password']) && $settings['call_password'] == 'Y' &&
                    fn_get_addon_version('cp_sms_services') >= 1.5 && 
                    in_array($sms_service, ['sms_ru', 'smsprofi_ru'])
                ) {               
                    $call_password = fn_cp_sms_send_call_password($phone);
                    if (!empty($call_password)) {
                        $code = $call_password;
                        $code_sended = true;
                    }
                } else {
    ...
    if (defined('CP_OTP_DEMO_MODE')) {
        fn_set_notification('W', __('important'), __('cp_otp_your_code_is') . ' ' . $code);
        $code_sended = true;
    }
    if ($code_sended) {
        if (defined('AJAX_REQUEST')) {
            Tygh::$app['ajax']->assign('cp_otp_code_sended', true);
        }
...
function fn_cp_otp_validate_code($code, $type = 'register', &$user_id = 0)
{
    $settings = Registry::get('addons.cp_otp_registration');
...
function fn_install_cp_otp_registration()
{
    ...
    foreach ($profile_fields as $section => $profile_field) {
        foreach ($profile_field as $field_id => $v) {
            if (in_array($v['field_name'], ['b_phone', 's_phone'])) {
                $p_field_data = [
                    'checkout_required' => 'N',
                    'checkout_show'     => 'N',
                    'profile_required'  => 'N',
...
function fn_cp_otp_registration_data_texts()
{   
    $lang_var = 'cp_otp_registration.create_user_text';
    $str = __('cp_otp_registration_text_title') . ': ';
    $url = fn_url('languages.translations?q=' . $lang_var);
    $str .= "{$lang_var} <a href=\"{$url}\" target=\"_blank\">" . __('edit') . "</a><br/><br/>";
    return $str;
}
