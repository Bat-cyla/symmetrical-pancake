...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if ($mode == 'login') {
        ...
        if (defined('AJAX_REQUEST')) {
            if (!empty($_REQUEST['return_url']) && isset($_REQUEST['obj_id'])) {
                $_POST['success_return_url'] = $_REQUEST['success_return_url'] = $_REQUEST['return_url'];
                $_POST['redirect_url'] = $_REQUEST['redirect_url'] = 'auth.login_form?cp_otp_message=1&obj_id=' . $_REQUEST['obj_id'];
            }
        }
    }
    return; 
