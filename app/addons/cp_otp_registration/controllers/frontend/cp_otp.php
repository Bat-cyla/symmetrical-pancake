...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($mode == 'pre_verification') {
        ...
        if (!empty($exists_user_id) && $exists_user_id != $user_id) {
            ...
            if (Registry::get('addons.recaptcha.status') == 'A') {
                ...
                if (!empty($addon_use_for_value['cp_otp']) && $addon_use_for_value['cp_otp'] != 'none') {
                    Tygh::$app['ajax']->assign('cp_show_code_popup', true);
                }
            }
        }
        exit;
