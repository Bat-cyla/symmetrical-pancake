...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if ($mode == 'update') {
        if (empty($auth['user_id'])) { // user register
            $login_type = Registry::get('addons.cp_otp_registration.login_type');
            if (empty($_REQUEST['user_data']['phone'])
                || !fn_cp_otp_check_verified($_REQUEST['user_data']['phone'], 'register')
            ) {
                fn_set_notification('E', __('error'), __('cp_otp_phone_need_confirm'));
        ...
        if (!empty($_REQUEST['user_data']['phone']) && isset($_REQUEST['verified_phone']) && $_REQUEST['verified_phone'] != $_REQUEST['user_data']['phone']) {
            $_REQUEST['user_data']['cp_phone_verified'] = $_POST['user_data']['cp_phone_verified'] = 'N';
        }
    ...
    if ($mode == 'cp_otp_login') {
        ...
        if (empty($_REQUEST['cp_otp_code']) || !fn_cp_otp_validate_code($_REQUEST['cp_otp_code'], 'login', $user_id)) {
            fn_set_notification('E', __('error'), __('cp_otp_fail_verification'));
            return fn_cp_otp_controller_do_redirect('auth.login_form');
        }

        if (empty($user_id)) {
            if (empty($auth['user_id']) && fn_cp_otp_allow_fast_registration())
            {

                $password = fn_cp_otp_generate_password(8);
                $_REQUEST['user_data']['password1'] = $_REQUEST['user_data']['password2'] = $password;
                if(Registry::get('addons.cp_otp_registration.use_fake_email') == YesNo::YES) {
                if (empty($_REQUEST['user_data']['email'])) {
                    $_REQUEST['user_data']['email'] = fn_checkout_generate_fake_email_address($_REQUEST['user_data'], TIME);
                    }
                }
    ...
    if ($mode == 'cp_check_otp') {
        $timeout = Registry::get('addons.cp_otp_registration.new_send_time');

        $session=Tygh::$app['session']['cp_otp'];
        if(!empty($session)){
            $time=$session['login']['time'];
            if(!fn_cp_otp_registration_check_timeout($time, $timeout)){
                fn_set_notification('W', __('warning'), __('cp_otp_not_enable_to_send_another_code_notification'));
                exit;
            }
        }
...
if ($mode == 'cp_phone_verification') {
    ...
    if (!empty($exists_user_id) && $exists_user_id != $user_id && empty($_REQUEST['go_skip_exist'])) {
        fn_set_notification('E', __('error'), __('cp_exist_phone_error'));
        return fn_cp_otp_controller_do_redirect('profiles.add', false, true);
    } else {
        ...
        if (defined('AJAX_REQUEST')) {
            Tygh::$app['ajax']->assign('obj_id', !empty($_REQUEST['obj_id']) ? $_REQUEST['obj_id'] : '');
        }
        Tygh::$app['view']->assign('obj_id', !empty($_REQUEST['obj_id']) ? $_REQUEST['obj_id'] : '');
        Tygh::$app['view']->assign('otp_type', $otp_type);
