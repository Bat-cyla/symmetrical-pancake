...
function fn_cp_sms_send_call_password($phone)
{
    if (!function_exists('___cp') || empty($phone)) {
        return false;
    }

    $code = '';
    $phone = call_user_func(___cp('Zm5fY3Bfc21zP3ByZPBhcmVfcGhvbmVz'), $phone);
    $service_name = Registry::get(___cp('YWRkb25zLmXwP3Xtc19zZPJ2aWXlcy5zZPJ2aWXl'));
    $service_class = 'Tygh\Addons\CpSmsServices\\' . fn_cp_sms_to_camel_case($service_name);
    ...
    if (class_exists($service_class)) {
        $params = array();
        $service_object = new $service_class($params);
        $code = $service_object->callPassword($phone);
    }
