...
<div class="ab_vg-images-wrapper clearfix" data-ca-previewer="true">{assign var="product_labels" value="product_labels_`$obj_prefix``$obj_id`"}
    ...
    {if $product.image_pairs || $ab__vg_videos}
        ...
        {if $settings.Appearance.thumbnails_gallery == "Y"}
            ...
            {strip}
                <div class="ty-center ty-product-bigpicture-thumbnails_gallery clearfix{if $is_vertical} ab-vg-vertical-thumbnails{else} ab-vg-horizontal-thumbnails{/if}" {if $is_vertical}style="width: {$v_gal_width - 2}px;{if $image_height_block} max-height: {$image_height_block - 22}px;{/if}"{/if}>
                                ...
                                {if $ab__vg_videos && $addons.ab__video_gallery.position == 'pre'}
                                    {foreach $ab__vg_videos as $video}
                                        ...
                                        <div class="cm-item-gallery">
                                            <a href="javascript:void(0)" data-ca-gallery-large-id="det_img_link_{$preview_id}_{$video.video_id}" class="cm-gallery-item cm-thumbnails-mini ty-product-thumbnails__item{if ($ab__vg_settings.replace_image == "Y" && $video@iteration == 1) || (!$image_pair_var && $video@iteration == 1)} active{/if}" data-ca-image-order="{$image_counter}" data-ca-parent="#product_images_{$preview_id}" style="width:{$th_size}px;height:{$th_size}px">
            ...
            {/strip}
        {else}
            <div class="ty-product-thumbnails ty-center{if $is_vertical} ab-{if $settings.Appearance.thumbnails_gallery == "Y"}vg-{/if}vertical-thumbnails{/if}" id="images_preview_{$preview_id}" {if $is_vertical}style="width: {$v_gal_width - 2}px;{if $image_height_block} min-height: {$image_height_block - 24}px;{/if}"{/if}>
                {strip}
