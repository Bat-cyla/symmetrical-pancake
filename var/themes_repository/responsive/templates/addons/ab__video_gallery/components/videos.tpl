{if $ab__vg_videos}
    {foreach $ab__vg_videos as $video}
        ...
        {else}
            <a id="det_img_link_{$preview_id}_{$video.video_id}" class="hidden ty-previewer ab__vg-image_gallery_video ab-{$addons.ab__video_gallery.video_icon}-icon cm-dialog-opener" data-ca-target-id="ab__vg_video_{$video.video_id}" data-ca-dialog-title="{$video.title}" title="{$video.title}" rel="nofollow">
                {include file="addons/ab__video_gallery/components/thumbnail.tpl" video=$video width=$image_width height=$image_height}
            </a>
        {/if}
    {/foreach}
