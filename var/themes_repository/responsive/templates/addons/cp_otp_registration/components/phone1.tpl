<div class="cp-phone-verified-wrap" id="phone_verification_info_{$obj_id}">
    ...
    <div class="ty-control-group ty-shipping-phone cm-phone">
        ...
        <input type="text" id="phone" class="ty-input-text cm-focus cm-mask-phone cp-phone {if $county_selection=true}ty-input-append ty-input-append--phone-mask{/if}" maxlength="25" value="{if $phone}+{$phone}{elseif !$placeholder}{elseif $cp_cntr_code}+{$country_code}{/if}" data-ca-verification="phone_verification_info_{$obj_id}" name="{$inp_name}[phone]" autocomplete="n" {if $placeholder}placeholder="{$placeholder}"{/if}>
