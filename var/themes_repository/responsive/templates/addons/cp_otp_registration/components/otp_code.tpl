...
{if $addons.cp_otp_registration.login_type == "otp" && $addons.cp_otp_registration.call_password == "Y" && "cp_sms_services"|fn_get_addon_version >= 1.5 && in_array($addons.cp_sms_services.service,  array("sms_ru", "smsprofi_ru"))}
    {$call_password = true}
{/if}

{if !$no_info_text}
    <div class="cp-code-sended-text">{if $call_password}{__("cp_otp_code_sended_by_call")}{elseif $phone}{__("cp_otp_code_sended_text")}{elseif $email}{__("cp_otp_email_code_sended_text")}{/if}</div>
{/if}
<div class="ty-control-group clearfix">
    ...
    <div class="cp-otp-code-wrap">
        ...
        {/for}
        *}
        <input id="cp_otp_code" type="text" value="" name="cp_otp_code" class="" data-ca-count="{$digits_count}" size="12" maxlength="24" />
